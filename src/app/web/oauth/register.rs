...
pub async fn register_client(State(state): State<AppState>, Json(body): Json<RegisterRequest>) -> Response {
    if body.redirect_uris.is_empty() {
        return (StatusCode::BAD_REQUEST, Json(RegisterError {
    ...
    let db_response = sqlx::query("INSERT INTO oauth_clients (client_id, redirect_uris, client_name) VALUES ($1, $2, $3)")
        .bind(&client_id)
    ...
    match db_response {
        Ok(_) => (StatusCode::CREATED, Json(RegisterSuccess {
            client_id,
